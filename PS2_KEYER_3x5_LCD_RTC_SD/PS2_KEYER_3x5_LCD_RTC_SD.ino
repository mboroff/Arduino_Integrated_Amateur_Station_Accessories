/***************************************************************
  This sketch was put together using the examples from the SD library
  ...
  additional features so you do not have to remember what special keys arepress.
       ...
       1 - QSO RCV Info   % - Options
       2 - Send TNX       S - Snd CQ
       3 - My Sig Info    T - Tune
       4 - Write QSO Log  # - Chg CW Speed
       5 - Set UTC Clock  D - Decoder on/off
...
/*
 ...
 clock             "       5     19    "            (and to pin 3 on the Arduino board)
 data              "       1     22    "            (    "      4         "           )
...
#define switchPin 28 // tells decoder to decode when high
...
char ch;                                    // char for reading serial 3
char cwBuffer[22]  = "                    ";  // working buffer when using an LCD
...
*******************************************************/
 ...
 // rtc.setTime(4, 9, 0);     // Set the time to 12:00:00 (24hr format)
  //rtc.setDate(26, 2, 2015);   // Set the date to January 1st, 2014
   tft.setCursor(0, 10);
  tft.println("Version 05/17/2015");
 ...
 if (sdDisable == false) {
  ...
  delay(2000);
  tft.setCursor(0, 40);             // prevue the menu
  tft.fillScreen(BLUE);
  tft.setTextSize(2);
  tft.println("            Options");
  tft.println(" ");  
  tft.println(" 1 - QSO RCV Info   % - Options");
  tft.println(" 2 - Send TNX       S - Snd CQ");
  tft.println(" 3 - My Sig Info    T - Tune");
  tft.println(" 4 - Write QSO Log  # - Chg CW Speed");
  tft.println(" 5 - Set UTC Clock  D - Decoder on/off");
  delay(4000);             // Give user time to read the menu
  tft.setTextSize(3);  
  displaySetup();
}
...
#ifdef DEBUG 1            
     Serial.println("enableDecoder == true");
#endif
